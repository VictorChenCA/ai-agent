    ...
    async def on_message(self, message: discord.Message):
                ...
                if attachment.filename.lower().endswith(".pdf"):
                    ...
                    try:
                        ...
                        if not os.path.exists(file_path):
                            logger.error(
                                f"❌ File was not saved correctly: {file_path}")
                            await message.channel.send("❌ Error saving file. Please try again.")
                            return

                        logger.info(
                            f"✅ PDF successfully saved at: {file_path}")
        ...
        if "awaiting_question_count" in self.study_agent.sessions.get(user_id, {}):
            try:
                ...
                terms = self.study_agent.extract_study_terms(
                    user_id, extracted_text)
                response = self.study_agent.start_session(
                    user_id, "")  # Reuse existing flow
        ...
        if content:
            # If the message does not contain a comma, treat it as a subject
            if ',' not in content:
                response = self.study_agent.start_session(
                    user_id, content)
            ...
            if "format" in session:
                ...
                if cur_term:
                    if session["format"] == "Multiple Choice":
                        ...
                        question = self.study_agent.generate_fill_in_the_blank_question(
                            cur_term)
        ...
        response = self.study_agent.check_answer(
            user_id, term, content, mcq_options)
        await message.channel.send(response)
